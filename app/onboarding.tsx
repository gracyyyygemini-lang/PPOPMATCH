  ...
  return (
    <View style={styles.ratingRow}>
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <Text style={[styles.ratingNum, { color: n <= value ? Colors.white : Colors.textMuted }]}>
            {n}
          </Text>
        </TouchableOpacity>
      ))}
    ...
    <View style={pStyles.wrapper}>
      ...
      <View style={[pStyles.bucket, { opacity: atHome ? 1 : 0.32 }]}>
        <View
          style={[
            pStyles.bucketInner,
            { backgroundColor: atHome ? Colors.ponyo + "22" : Colors.teal + "14" },
          ]}
        >
...
export default function OnboardingScreen() {
  ...
  const isStepComplete = (s: number): boolean => {
    switch (s) {
      case 1:
        return name.trim().length > 0 && !!profileImage && !!major && bio.trim().length > 0;
      case 2:
        return !!intent;
      case 3:
        return true;
  ...
  const handleFinish = async () => {
    ...
    const user: UserProfile = {
      ...
      gender,
      genderPref: isSupplySide ? "mixed" : genderPref,
    ...
    switch (intent) {
      ...
      case "sublessee": {
        ...
        router.replace((sublesseeRoutes[leaseType] || "/marketplace?category=short_term") as any);
        break;
      }
      default:
        router.replace("/(tabs)");
        ...
        {step === 1 && (
          <View style={styles.stepContainer}>
            ...
            <TextInput
              ...
              returnKeyType="next"
            />
            {!name.trim() && <Text style={styles.fieldHint}>Required to continue</Text>}
            ...
            <TouchableOpacity style={styles.input} onPress={() => setShowMajorPicker(!showMajorPicker)}>
              ...
              <Ionicons
                name={showMajorPicker ? "chevron-up" : "chevron-down"}
                size={15}
            ...
            {showMajorPicker && (
              <View style={styles.majorList}>
                {MAJORS.map((m) => (
                  <TouchableOpacity
                    key={m}
                    style={[styles.majorItem, major === m && styles.majorSelected]}
                    onPress={() => {
                      setMajor(m);
        ...
        {step === 2 && (
          <View style={styles.stepContainer}>
            ...
            {([
              ...
              <TouchableOpacity
                key={key}
                style={[styles.optionCard, intent === key && styles.optionCardActive]}
                onPress={() => {
                  setIntent(key);
                  Haptics.selectionAsync();
                }}
          ...
          <View style={styles.stepContainer}>
            ...
            <View style={styles.chipRow}>
              {([
                ...
                <TouchableOpacity
                  key={k}
                  style={[styles.chip, genderPref === k && styles.chipActive]}
                  onPress={() => {
                    setGenderPref(k);
                    Haptics.selectionAsync();
                  }}
            ...
            {([
              ...
              <TouchableOpacity
                key={k}
                style={[styles.optionCard, leaseType === k && styles.optionCardActive]}
                onPress={() => {
                  setLeaseType(k);
                  Haptics.selectionAsync();
                }}
        ...
        {step === 4 && (
          <View style={styles.stepContainer}>
            ...
            {ZONES.map((z) => (
              <TouchableOpacity
                key={z.key}
                style={[styles.optionCard, zone === z.key && styles.optionCardActive]}
                onPress={() => {
                  setZone(z.key);
                  Haptics.selectionAsync();
                }}
        ...
        {step === 5 && (
          <View style={styles.stepContainer}>
            ...
            <View style={styles.toggleRow}>
              ...
              <TouchableOpacity
                style={[styles.toggle, staysThanksgiving && styles.toggleOn]}
                onPress={() => {
                  setStaysThanksgiving(!staysThanksgiving);
                  Haptics.selectionAsync();
                }}
              >
            ...
            <View style={styles.toggleRow}>
              ...
              <TouchableOpacity
                style={[styles.toggle, staysSpringBreak && styles.toggleOn]}
                onPress={() => {
                  setStaysSpringBreak(!staysSpringBreak);
                  Haptics.selectionAsync();
                }}
              >
...
const styles = StyleSheet.create({
  ...
  toggle: {
    ...
    paddingVertical: 7,
    borderRadius: 16,
    backgroundColor: Colors.foamDark,
    borderWidth: 1.5,
    borderColor: Colors.borderSoft,
  },
  ...
  majorItem: {
    ...
    paddingVertical: 11,
    borderBottomWidth: 1,
    borderBottomColor: Colors.foamDark,
  },
